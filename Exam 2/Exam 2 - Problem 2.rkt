...
#reader(lib "htdp-intermediate-lambda-reader.ss" "lang")((modname |Exam 2 - Problem 2|) (read-case-sensitive #t) (teachpacks ()) (htdp-settings #(#t constructor repeating-decimal #f #t none #f () #f)))
; == Exam 2, Problem 2 ==
...
; bytes-on-disk : Directory -> NatNumber
; Interpretation: counts the total size of a supplied directory
(define (bytes-on-disk dir)
  (local [; get-sizes : Directory -> [List-of-Numbers]
          ; Interpretation: creates a list with all of
          ; a directory's file sizes
          (define (get-sizes dir)
            (map (λ (d) (file-size d)) (dir-files dir)))]
    (foldr + 0 (get-sizes dir))))
              
(check-expect (bytes-on-disk DIR-CS2500) 888)
(check-expect (bytes-on-disk DIR-CS3200) 613)
...
; keep-file-if : Directory Function -> Directory
; Interpretation: returns a modified directory containing
; only files that are true to the Function
