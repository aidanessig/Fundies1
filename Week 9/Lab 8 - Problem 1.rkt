...
#reader(lib "htdp-intermediate-lambda-reader.ss" "lang")((modname |Lab 8 - Problem 1|) (read-case-sensitive #t) (teachpacks ()) (htdp-settings #(#t constructor repeating-decimal #f #t none #f () #f)))
(require 2htdp/image)
...
(define (f x y z)
  (local [(define x (+ y z))] ; x 3
    (local [(define y z)] ; y 1
      (local [(define z (* x y))] ; z 3
        (+ x y z))))) ; + 3 1 3
 
(define (g a b)
  (+ (local [(define b (string->number a))] ; b 5 | -5
       (local [(define a (sqr b))] ; a 25 | 25
         (- a b))) ; 20 | 30
     (string-length a) ; 1 | 2
     (if b 10 5))) ; 10 | 5
...
; adder-local : Number -> [Number Number -> Number]
; Interpretation: adds the input of function and its output
(define (adder-local a)
  (local [(define (add-again b)
            (+ a b))]
    add-again))

; adder-lambda : Number -> [Number Number -> Number]
; Interpretation: adds the input of function and its output
(define (adder-lambda a)
  (λ (b) (+ a b)))
...
; point-mover : Number Number -> (make-posn)
; Interpretation: adjusts x or y values of a given
; move-right or move-down function
(define (point-mover n1 n2)
  (λ (posn) (make-posn (+ (posn-x posn) n1)
                       (+ (posn-y posn) n2))))
...
; elbow-mover : Number Number Number Number -> [NatNumber POSN] -> POSN
; Interpretation: FIRST TWO GO INTO POINT-MOVER
(define (elbow-mover r1 r2 r3 r4)
  (λ (n posn) (cond
                [(< (remainder n 3) 2) ((point-mover r1 r2) posn)]
                [else ((point-mover r3 r4) posn)])))
